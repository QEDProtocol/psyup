...
# genesis_contracts.json is gitignored but required at build time by
# psy_config's build.rs (it panics if the file is missing). Seed an empty
# array so the compiler builds; `make gen-deploy-json` below regenerates the
# real one (cargo reruns build.rs via rerun-if-changed) before parth is built.
if [ ! -f "$parth_dir/genesis_contracts.json" ]; then
    echo "==> Seeding empty genesis_contracts.json in $(basename "$parth_dir") (regenerated by gen-deploy-json)"
    printf '[]\n' > "$parth_dir/genesis_contracts.json"
fi
